...
class ImageSubscriber(Node):
    ...
    def add_small_pictures(self, img, small_images):
        ...
        x_base_offset = thumb_w // 4
        y_base_offset = max(thumb_h // 10, 5)
        ...
        for small in small_images:
            ...
            y_end = min(y_offset + size[1], img_h)
            x_end = min(x_offset + size[0], img_w)

            if x_offset >= img_w or y_offset >= img_h:
                break

            img[y_offset:y_end, x_offset:x_end] = small[:y_end - y_offset, :x_end - x_offset]
